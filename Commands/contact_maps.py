...
class DistanceMapBuilder:
    def __init__(self, atom="CA", verbose=True, pedantic=True, glycine_hack=-1):
        ...
        for chain_name in structure_container.chains:
            ...
            if chain['seqres-seq'] is not None and len(chain['seqres-seq']) > 0:
                ...
                if self.pedantic and sanity_check != final_seq_one_letter_codes:
                    ...
                    missing_residue_heuristic_2 = final_seq_one_letter_codes.count('-') / len(
                        final_seq_one_letter_codes)
                    if count == non_canonicals_or_het:
                        ...
                        print(
                            f"Warning: The final sequence and the sanity check were different. Over 50% of the chain is unresolved. Nothing we can do about it.")
                    else:
                        print("Vlada")
                        # raise Exception(
                ...
                contact_maps.with_map_for_chain(chain_name,
                                                self.__residue_list_to_contact_map(final_residue_list,
                                                                                   len(corrected_atom_seq)))
