...
def compute_polar_coordinates(mesh, do_fast=False, radius=12, max_vertices=200):
    """
...
def output_patch_coords(subv, subf, subn, i, neigh_i, theta, rho,optional_name=None):
    """ 
    ...
    #mesh.add_attribute('charges')
    #mesh.set_attribute('charges', charge)
    if optional_name is not None:
        pymesh.save_mesh('/home/felix/v{}.ply'.format(optional_name), mesh, *mesh.get_attribute_names(), use_float=True, ascii=True)

#@jit
def call_mds(mds_obj, pair_dist):
    return mds_obj.fit_transform(np.array(pair_dist))
