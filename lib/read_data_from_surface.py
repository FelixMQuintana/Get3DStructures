...
def read_data_from_surface(ply_fn, params, vertex_idx_of_interest):
    """
    ...
    charge = mesh.get_attribute("vertex_charges")
    ...
    # n: number of patches, equal to the number of verteces of interest.
    n = vertex_idx_of_interest
    #n= len(mesh.vertices)
    #input_feat = np.zeros((n * params['max_shape_size'], 10))
    input_feat = np.zeros((len(n)*params['max_shape_size'],10))
   # input_feat = np.zeros((n, params['max_shape_size'], 11))
    new_faces = []
    # Compute the input features for each patch.

    #for vix in range(n):
    for index, vix in enumerate(n):
        # Patch members.
     ...
     #   ddc = compute_ddc(patch_v, patch_n, mesh.vertices[cp_vertex_idx], patch_rho)

        #input_feat[vix*100:vix * 100 + len(neigh_vix), 0] = si[neigh_vix]
    #   input_feat[index*len(neigh_vix):(index+1)*len(neigh_vix), 1] = ddc
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 1] = hbond[neigh_vix]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 2] = charge[neigh_vix]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 3] = hphob[neigh_vix]
    #     input_feat[vix, 3, 5] = mesh.vertices[vix]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 4] = patch_v[:,0]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 5] = patch_v[:,1]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 6] = patch_v[:,2]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 7] = patch_n[:,0]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 8] = patch_n[:,1]
        #input_feat[vix*100:vix * 100 + len(neigh_vix), 9] = patch_n[:,2]




        input_feat[index*100:index * 100 + len(neigh_vix), 0] = si[neigh_vix]
     #   input_feat[index*len(neigh_vix):(index+1)*len(neigh_vix), 1] = ddc
        input_feat[index*100:index * 100 + len(neigh_vix), 1] = hbond[neigh_vix]
        input_feat[index*100:index * 100 + len(neigh_vix), 2] = charge[neigh_vix]
        input_feat[index*100:index * 100 + len(neigh_vix), 3] = hphob[neigh_vix]
   #     input_feat[vix, 3, 5] = mesh.vertices[vix]
        input_feat[index*100:index * 100 + len(neigh_vix), 4] = patch_v[:,0]
        input_feat[index*100:index * 100 + len(neigh_vix), 5] = patch_v[:,1]
        input_feat[index*100:index * 100 + len(neigh_vix), 6] = patch_v[:,2]
        input_feat[index*100:index * 100 + len(neigh_vix), 7] = patch_n[:,0]
        input_feat[index*100:index * 100 + len(neigh_vix), 8] = patch_n[:,1]
        input_feat[index*100:index * 100 + len(neigh_vix), 9] = patch_n[:,2]
       # faces_of_interest = []
  #      cond = np.isin(mesh.faces, neigh_indices[vix]).nonzero()
  #      counts = np.unique(cond[0], return_counts=True)
  #      counts_array = np.array([counts[0], counts[1]])
  #      valid_face_indexs = counts_array[0, counts_array[1,:] == 3]
  #      faces_old_vertex = mesh.faces[valid_face_indexs]
  #      old_vertex_to_new_dict = {}
  #      count = 0
  #      for element in neigh_indices[vix]:
  #          old_vertex_to_new_dict[element] = count
  #          count += 1
  #      new_vertex_list = []
       # np.where(faces_old_vertex==neigh_indices[vix])
  ...
  #      new_faces.append(numpy.array(new_vertex_list))



    return input_feat, rho, theta, mask, neigh_indices, iface_labels, np.copy(mesh.vertices), #new_faces
...
def compute_ddc(patch_v, patch_n, patch_cp, patch_rho):
    """
        Compute the distance dependent curvature, Yin et al PNAS 2009
            ...
            patch_cp: the vertex of the central point of the patch
   ...
   # i = patch_cp
    ...
    dij = np.linalg.norm(r - patch_cp, axis=1)
